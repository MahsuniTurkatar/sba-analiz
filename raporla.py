...
def load_data():
    try:
        ...
        # Sütun isimlerindeki gizli boşlukları ve karakterleri TEMİZLE (Kritik Hata Giderici)
...
# --- CSS: BEYAZ BAŞLIKLAR VE KESİN ORTALAMA ---
st.markdown("""
    <style>
    ...
    /* TABLO TASARIMI: HER ŞEYİ ORTALAMA */
    ...
    /* TOPLAM SATIRI: LACİVERT ZEMİN, SARI YAZI */
...
# SIFIRLARI GİZLEME FONKSİYONU
def hide_zeros(df):
    return df.mask(df == 0, "").mask(df == "0", "").mask(df == 0.0, "")

st.markdown("<h1>Sağlık Bilimleri Araştırma Etik Kurulu Başvuruları</h1>", unsafe_allow_html=True)

# --- 1. ANA METRİKLER (190 ve 4) ---
...
if df_gundem is not None:
    ...
    df_g_final = df_g_final[df_g_final['Toplam'] > 0] # Sıfır olan gündemleri gösterme
    df_g_final['Gündem Tarihleri'] = pd.to_datetime(df_g_final['Gündem Tarihleri']).dt.strftime('%d.%m.%Y')
    
    html_g = hide_zeros(df_g_final).to_html(index=False, classes='styled-table')
...
with tab1:
    ...
    if df_raportor is not None:
        ...
        html_c = hide_zeros(pd.DataFrame(c_dict)).to_html(index=False, classes='styled-table')
...
with tab2:
    ...
    if df_raportor is not None:
        ...
        sr = st.selectbox("Raportör Seçin:", r_list)
