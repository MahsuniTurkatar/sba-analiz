...
/* ── Streamlit nav butonlarını gizle ── */
...
# ── NAVİGASYON ────────────────────────────────────────────────────────────────
nav_html = '<div class="nav-wrap"><div class="nav-bar">'
for i, (yazi, ikon) in enumerate(MENU):
    cls = "nav-btn active" if i == aktif else "nav-btn"
    nav_html += f'<span class="{cls}" data-idx="{i}"><span class="nav-ikon">{ikon}</span>{yazi}</span>'
nav_html += '</div></div>'
st.markdown(nav_html, unsafe_allow_html=True)

# Gerçek Streamlit butonları (CSS ile tamamen gizli, sadece tıklama yakalama)
cols = st.columns(len(MENU))
for i, (yazi, ikon) in enumerate(MENU):
    with cols[i]:
        if st.button(f"{ikon} {yazi}", key=f"nav_{i}"):
