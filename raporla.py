...
def load_all_data():
    try:
        # Sayfaları yüklediğin dosyaya göre okuyoruz
...
# --- CSS: BEYAZ BAŞLIKLAR, LACİVERT TOPLAM VE TAM ORTALI DÜZEN ---
st.markdown("""
    <style>
    ...
    /* METRİKLER: Beyaz Başlık, Sarı Değer */
    [data-testid="stMetricLabel"] { color: #FFFFFF !important; justify-content: center !important; text-align: center !important; display: flex !important; font-size: 1.1rem !important; }
    [data-testid="stMetricValue"] { color: #FEDD00 !important; text-align: center !important; font-weight: bold !important; }
    ...
    div[data-testid="stMetric"] {
        ...
        width: 180px !important; margin: auto !important; padding: 10px !important;
    }
    
    /* TABLO TASARIMI: Hepsi Ortalı */
    .table-wrapper { display: flex; justify-content: center; width: 100%; margin: 5px 0; }
    .styled-table { width: auto !important; border-collapse: collapse; color: white; font-size: 0.9rem; margin: auto; }
    ...
    /* Sekmeler */
    .stTabs [data-baseweb="tab"] p { color: #FFFFFF !important; font-weight: bold; font-size: 1rem; }
    hr { border: 0.5px solid #FEDD00; opacity: 0.3; }
    .footer { text-align: center; color: #FFFFFF; padding: 20px; border-top: 1px solid #FEDD00; margin-top: 30px; font-weight: bold; }
    </style>
    """, unsafe_allow_html=True)

# Sayfada sıfırları gizleme fonksiyonu (HTML render öncesi)
def format_zero(val):
...
# --- 2. GÜNDEM SAYILARI (SIFIRSIZ VE ORTALI) ---
if df_gundem is not None:
    ...
    # Sıfır içeren satırları (boş olanları) temizle
    df_g_final = df_g_final[df_g_final['Toplam'] > 0]
    df_g_final['Gündem Tarihleri'] = pd.to_datetime(df_g_final['Gündem Tarihleri'], errors='coerce').dt.strftime('%d.%m.%Y')
    
    html_g = df_g_final.applymap(format_zero).to_html(index=False, classes='styled-table')
...
with tab1:
    ...
    if df_raportor is not None:
        try:
            ...
            html_c = df_c.applymap(format_zero).to_html(index=False, classes='styled-table')
        ...
        except: st.error("Çizelge oluşturulurken hata oluştu.")
...
with tab2:
    ...
    if df_raportor is not None:
        ...
        sec_r = st.selectbox("Analiz Edilecek Raportörü Seçin:", r_names)
        ...
        # Raportör tablosunda sıfırları kaldır
        html_r = pd.DataFrame(rd_data).applymap(format_zero).to_html(index=False, classes='styled-table').replace('<td>📊 TOPLAM</td>', '<td class="total-row">📊 TOPLAM</td>')
        st.markdown(f'<div class="table-wrapper">{html_r}</div>', unsafe_allow_html=True)
        
        # Yakınlaştırılmış Metrikler
        st.markdown("<br>", unsafe_allow_html=True)
        rm_l, rm1, rm2, rm3, rm_r = st.columns([2, 1, 1, 1, 2])
...
with tab3:
    ...
    if df_pivot is not None:
        # Pivot sayfasının sol tarafı (Birimler)
        b_df = df_pivot.iloc[:, [0, 1]].dropna().copy()
        b_df.columns = ["Birim Adı", "Dosya Sayısı"]
        b_df = b_df[~b_df["Birim Adı"].astype(str).str.contains("Etiketleri|Toplam|Genel", na=False)]
...
with tab4:
    ...
    if df_pivot is not None:
        # Pivot sayfasının sağ tarafı (Sorumlular)
        s_df = df_pivot.iloc[:, [3, 4]].dropna().copy()
        s_df.columns = ["Sorumlu Araştırmacı", "Dosya Sayısı"]
        s_df = s_df[~s_df["Sorumlu Araştırmacı"].astype(str).str.contains("Etiketleri|Toplam|Genel", na=False)]
