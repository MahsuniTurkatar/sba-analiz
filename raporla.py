...
# --- CSS: KESİN VE NET DÜZEN ---
st.markdown("""
    <style>
    .stApp { background-color: #000814; }
    h1, h2, h3 { color: #FFFFFF !important; text-align: center !important; }
    
    /* METRİKLER MERKEZDE VE YAN YANA */
    [data-testid="stHorizontalBlock"] { justify-content: center !important; gap: 10px !important; }
    ...
    /* TABLOLAR: İĞNELENMİŞ GENİŞLİK */
    .table-wrapper { display: flex; justify-content: center; width: 100%; margin: 15px 0; }
    .styled-table { 
        margin: auto; border-collapse: collapse; color: white; font-size: 0.8rem; 
        width: auto !important; table-layout: auto !important; 
    }
    .styled-table th { background-color: #001d3d; color: #FEDD00 !important; border: 1px solid #FEDD00; padding: 8px 10px; text-align: center !important; white-space: nowrap; }
    .styled-table td { border: 1px solid #FEDD00; padding: 6px 8px; text-align: center !important; white-space: nowrap; }
...
# --- 1. METRİKLER (206 ve 5 - TAM ORTADA) ---
c_m1, c_m2 = st.columns([1, 1])
with c_m1: st.metric("Toplam Başvuru", "206")
with c_m2: st.metric("Kurul Sayısı", "5")

# --- 2. GÜNDEM SAYILARI (BOZULMADI) ---
...
with t1:
    if df_raportor is not None:
        ...
        # Excel'deki tüm sütunları (Onay, Düzeltme, KAEK vb.) temiz haliyle alıyoruz
        html_genel = clean_df(df_raportor.dropna(subset=['Adı Soyadı'])).to_html(index=False, classes='styled-table')
        html_genel = html_genel.replace('<td>TOPLAM</td>', '<td class="total-row">TOPLAM</td>')
        st.markdown(f'<div class="table-wrapper">{html_genel}</div>', unsafe_allow_html=True)
...
with t2:
    if df_raportor is not None:
        ...
        # İKONLU ANALİZ TABLOSU
        rd = {
            "Kategori": ["📜 Bireysel Araştırma", "🎓 Yüksek Lisans Tezi", "📚 Doktora Tezi", "🩺 Uzmanlık Tezi", "⭐ GENEL TOPLAM"],
        ...
        # ATANAN - KARAR - BEKLEYEN KUTULARI
...
with t3:
    if df_pivot is not None:
        ...
        # Toplam Satırı (Gündem ile tutarlı 206)
